...
public class Main {
  ...
  class signAlgo {
    ...
    long invK() {
    for (int x = 1; x < p - 1; x++)
        if ((k * x) % (p - 1) == 1) return x;
    return 1;
}
long createS1(long b, long c) {
    long a = ((long) Math.pow(b, c));
    if (a < (long) Math.pow(2, 36) - 1) return a % p;
    else(a == (long) Math.pow(2, 36) - 1) return (createS1(b, c / 2) * createS1(b, c - c / 2)) % p;
}
long createS2() {
...
class verify {
    ...
    long v1(long b, long c, long d, long e) {
        ...
        if (a < (long) Math.pow(2, 36) - 1) return a % p;
        else(a == (long) Math.pow(2, 36) - 1) return (v1(b, c / 2, d, e / 2) * v1(b, c - c / 2, d, e - e / 2)) % p;
    }
...
long v2(long b, long c) {
    long a = ((long) Math.pow(b, c));
    if (a < (long) Math.pow(2, 36) - 1) return a % p;
    else(a == (long) Math.pow(2, 36) - 1) return (v2(b, c / 2) * v2(b, c - c / 2)) % p;
}
void verified() {
